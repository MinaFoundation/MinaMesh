...
initialize_postgres() {
    sudo -u postgres /usr/lib/postgresql/13/bin/initdb -D "$POSTGRES_DATA_DIR" > /dev/null 2>&1
    sudo -u postgres /usr/lib/postgresql/13/bin/pg_ctl -D "$POSTGRES_DATA_DIR" -l "${POSTGRES_DATA_DIR}/postgresql.log" start > /dev/null 2>&1
}
