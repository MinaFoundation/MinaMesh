...
coinbase_receiver_info AS (
    ...
    INNER JOIN blocks_internal_commands AS bic_coinbase_receiver
        ON
            bic.block_id = bic_coinbase_receiver.block_id
            AND (
                bic.internal_command_id
                <> bic_coinbase_receiver.internal_command_id
                OR bic.sequence_no <> bic_coinbase_receiver.sequence_no
                OR bic.secondary_sequence_no
                <> bic_coinbase_receiver.secondary_sequence_no
            )
    INNER JOIN internal_commands AS ic_coinbase_receiver
        ON
            ic.command_type = 'fee_transfer_via_coinbase'
            AND ic_coinbase_receiver.command_type = 'coinbase'
            AND bic_coinbase_receiver.internal_command_id
            = ic_coinbase_receiver.id
    INNER JOIN public_keys AS coinbase_receiver_pk
...
internal_commands_info AS (
    SELECT DISTINCT ON (
        ...
        b.state_hash,
        b.height
    FROM internal_commands AS i
    INNER JOIN blocks_internal_commands AS bic
        ON i.id = bic.internal_command_id
    INNER JOIN public_keys AS pk
        ON i.receiver_id = pk.id
    INNER JOIN blocks AS b
        ON bic.block_id = b.id
    LEFT JOIN coinbase_receiver_info AS cri
        ON
            bic.block_id = cri.block_id
    ...
    WHERE
        ...
        AND ($2 = i.hash OR $2 IS NULL)
        AND (
            ($3 = pk.value OR $3 = cri.coinbase_receiver)
            AND $4 = '' OR ($3 IS NULL AND $4 IS NULL)
        )
        AND ($5 = bic.status OR $5 IS NULL)
        AND ($6 = bic.status OR $6 IS NULL)
        AND (($7 = pk.value OR $7 = cri.coinbase_receiver) OR $7 IS NULL)
),
...
FROM
    ...
    (
        SELECT *
        FROM internal_commands_info
        ORDER BY block_id, id, sequence_no, secondary_sequence_no
        LIMIT
            5
            OFFSET 0
    ) AS i
LEFT JOIN account_identifiers AS ai
    ON i.receiver_id = ai.public_key_id
LEFT JOIN accounts_created AS ac
    ON
        ai.id = ac.account_identifier_id
