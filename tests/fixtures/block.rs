...
pub fn block<'a>() -> CompareGroup<'a> {
  ...
  let blocks = blocks_by_index.into_iter().chain(blocks_by_hash.into_iter()).collect();
  ("/block", blocks)
}
...
pub fn block_not_found<'a>() -> CompareGroup<'a> {
  ("/block", vec![Box::new(BlockRequest {
    network_identifier: Box::new(network_id()),
    block_identifier: Box::new(PartialBlockIdentifier { index: None, hash: Some("not_founc".to_string()) }),
  })])
}
