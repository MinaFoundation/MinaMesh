...
async fn test_error_properties() {
  ...
  let cases = vec![
    ...
    (
      TransactionSubmitNoSender("No sender".to_string()),
      20,
    ...
    (
      TransactionSubmitDuplicate("Duplicate transaction".to_string()),
      21,
      "A duplicate transaction was detected.",
      false,
      StatusCode::CONFLICT,
    ),
    (TransactionSubmitBadNonce("Bad nonce".to_string()), 22, "The nonce is invalid.", false, StatusCode::BAD_REQUEST),
    (
      TransactionSubmitFeeSmall("Fee too small".to_string()),
      23,
    ...
    (
      TransactionSubmitInvalidSignature("Invalid signature".to_string()),
      24,
    ...
    (
      TransactionSubmitInsufficientBalance("Insufficient balance".to_string()),
      25,
    ...
    (
      TransactionSubmitExpired("Expired transaction".to_string()),
      26,
